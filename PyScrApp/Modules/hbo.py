...
movie_base = 'https://api-hbon.hbo.clearleap.com/cloffice/client/web/browse/ea73aabd-24a3-473e-8f3a-39aeb7f0f93e?max=20&language=fi_hbon&offset='
series_base = 'https://api-hbon.hbo.clearleap.com/cloffice/client/web/browse/f5dde064-495d-41dc-8cd7-cbb76baaf8d0?max=20&language=fi_hbon&offset='
scrape_data = {
    'type' : 'xml',
...
#     """ Check if the HBO service is available for a movie
#     """  
...
def hbo_scraper(cnx,cursor):
    ...
    hbo_base = scrape_data['movie_scrape_url']                    # base page 
    # add_hbo_to_steamingService(cnx, cursor, hbo_base)
    if not is_service_in_database(cursor, 'HBO'):
        add_service_to_steamingService(cnx, cursor, 'HBO', json.dumps(scrape_data))
            ...
            if omdb_data:
                ...
                if len(movie['year']) > 4:
                    ...
                    if  not is_title_year_in_db(cursor, movie): 
                        ...
                        if not is_service_available_for_movie(cursor, get_streamingServiceId(cursor,'HBO'), movie):
...
def hbo_series_scraper(cnx, cursor):
    """
    Scraper from HBO NORDIC tv series
    """

    hbo_base = scrape_data['tv_series_scrape_url']
    if not is_service_in_database(cursor, 'HBO'):
        add_service_to_steamingService(cnx, cursor, 'HBO', json.dumps(scrape_data))
    ...
    while flag:
        ...
        for elem in content.select('item'):             #cycles the <item> elements in the page
            ...
            print("Scraped tv series title: ", par[0]," from hbo")
            failed = False
            omdb_data = omdb_search_without_year(par[0])      #search for the movie on the Online Movie DataBase
            # print(par[0], omdb_data)
            if failed:                                          #BUG it doesn't get inside this loop
                    print(par[0],'failed to be found on OMDB!')
